...
!       simulation parameters
        ...
        logical :: rflag ! rotate domain when sampling streamline sections?
...
!$omp   shared(mode, Ofile, Dm, tmax, slsmax, csf, rflag, np, nt, tl, &
!$omp   n, dx, v, g, visited, xp, l, caseno)
!       initialize things
...
!           streamline tracking
            ...
            pflag = pollock(d, n, dx, v, g, bnd, ic, xsi, Dm, 0, real(tmax-tc), &
                            m, dt, xi, u, a, ns, trace, xe, idum)
            if (mode == 0) print '(i0)', m
